    ...
    <div className="min-h-screen bg-harar-cream">
      <div className="fixed left-0 top-0 h-full w-64 bg-white shadow-lg z-50">
        <div className="p-6">
          ...
          <p className="text-sm text-harar-brown/60">John Doe</p>
        </div>
          ...
          {activeTab === 'overview' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <MetricCard title="Total Orders" value={buyerOrders.length} icon={ShoppingCart} />
                <MetricCard title="Total Spent" value={formatCurrency(totalSpent)} icon={TrendingUp} />
                <MetricCard title="Wishlist Items" value={wishlistItems.length} icon={Heart} />
                <MetricCard title="Pending Deliveries" value={buyerOrders.filter(o => o.status === 'shipped').length} icon={Package} />
              </div>
                ...
                <div className="space-y-4">
                  {buyerOrders.slice(0, 3).map((order) => (
                    <div key={order.id} className="flex items-center justify-between p-4 bg-harar-sand/30 rounded-xl">
                      <div>
                        <p className="font-semibold text-harar-brown">{order.id}</p>
                        <p className="text-sm text-harar-brown/70">{order.products.length} items • {formatDate(order.createdAt)}</p>
                      </div>
                    ...
                    <tbody>
                      {buyerOrders.map((order) => (
                        <tr key={order.id} className="border-b border-harar-sand/50 hover:bg-harar-sand/20">
                          ...
                          <td className="py-4 px-4 text-harar-brown/70">{formatDate(order.createdAt)}</td>
